...
const { isAdminAuthenticated, analyze, search, processImage, preview } = vi.hoisted(() => ({
  isAdminAuthenticated: vi.fn(), analyze: vi.fn(), search: vi.fn(), processImage: vi.fn(), preview: vi.fn()
}));
vi.mock("@/lib/admin-auth", () => ({ isAdminAuthenticated }));
vi.mock("@/lib/http-security", () => ({ validateTrustedOrigin: () => null, validateJsonRequest: () => null }));
vi.mock("@/lib/image-assistant/source-provider", () => ({ isSafeImageSearchConfigured: () => false }));
vi.mock("@/services/imageAssistantService", () => ({
  computeCsvFingerprint: () => "abc123",
  createImageAssistantService: () => ({ analyze, search, process: processImage, preview })
}));
