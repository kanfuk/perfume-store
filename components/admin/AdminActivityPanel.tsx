...
export function AdminActivityPanel() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/activity?${query}`, { cache: "no-store" })
      .then(async (response) => {
        const payload = await response.json() as { items?: AuditItem[]; actions?: string[]; error?: string };
        if (!response.ok) throw new Error(payload.error ?? "No fue posible cargar la actividad.");
        if (active) { setItems(payload.items ?? []); setActions(payload.actions ?? []); setError(""); }
      })
  ...
  return <section className="space-y-4">
    ...
    <div className="grid gap-3 rounded-2xl border border-[#DDD0C1] bg-white p-4 sm:grid-cols-4">
