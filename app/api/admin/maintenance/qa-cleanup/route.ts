...
import { hasOnlyFields, isValidIdempotencyKey, QA_CLEANUP_CONFIRMATION } from "@/lib/mvp-maintenance";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!hasOnlyFields(body, ["confirmation", "idempotencyKey"]) ||
        body.confirmation !== QA_CLEANUP_CONFIRMATION || !isValidIdempotencyKey(body.idempotencyKey)) {
      return maintenanceJson({ error: "Confirmación o clave de idempotencia inválida." }, { status: 400 });
    }
    return maintenanceJson(await createMvpMaintenanceService().qaCleanup(body.idempotencyKey as string));
  } catch (error) {
