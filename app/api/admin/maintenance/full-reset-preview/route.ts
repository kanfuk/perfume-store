...
export async function GET(request: Request) {
  ...
  try { return maintenanceJson(await createMvpMaintenanceService().fullOperationalResetPreview()); }
  catch (error) { return maintenanceJson({ error: sanitizedMaintenanceError(error) }, { status: 400 }); }
}
